...
type Params = { modo?: string; erro?: string; enviado?: string; origem?: string };
const errors: Record<string, string> = {
  credenciais: 'Não foi possível entrar com esses dados. Confira o e-mail e a senha ou use “Esqueci minha senha”.',
  'email-nao-confirmado': 'Confirme seu e-mail pelo link de cadastro antes de entrar.',
  'tentativas-login': 'Muitas tentativas de login. Aguarde alguns minutos antes de tentar novamente.',
...
export default async function Home({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="auth-page">
    ...
    <div className="auth-layout">
      <div className="auth-story"><div className="auth-logo"><b>FV</b><span>Fazenda Varela IA</span></div><div className="auth-story-bottom"><span className="eyebrow-light">GESTÃO RURAL INTELIGENTE</span><h1>O campo em<br/><em>boas mãos.</em></h1><p>Rebanho, leite e finanças com a clareza que a sua fazenda merece.</p><div className="auth-caption">BREU BRANCO · PARÁ</div></div></div>
      <section className="auth-box"><div className="auth-mobile-brand">FAZENDA VARELA <b>IA</b></div>
        ...
        {params.erro && (params.erro !== 'limite-email' || params.origem === 'email' && (mode !== 'entrar' || params.modo === 'entrar')) && <p role="alert" className="auth-message bad">{errors[params.erro] || 'Não foi possível concluir. Tente novamente.'}</p>}
        {params.enviado && (mode !== 'entrar' || params.origem === 'email' || params.enviado === 'cadastro') && <p role="status" className="auth-message">{params.enviado === 'recuperacao' ? 'Se houver uma conta com esse e-mail, você receberá um link de recuperação.' : params.enviado === 'cadastro' ? 'Confira seu e-mail para confirmar a conta. Depois entre com sua senha.' : 'Enviamos um link de acesso. Abra o mais recente no seu e-mail.'}</p>}
