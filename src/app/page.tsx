...
import { createAnimal, createFarm, signIn, signInWithPassword, signOut } from './actions';
...
export default async function Home({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="shell">
    ...
    <div className="content"><div className="eyebrow">PAINEL DE CONTROLE <span> / VISÃO GERAL</span></div>
      ...
      {!user ? <section className="panel auth"><div><span className="label">ACESSO SEGURO</span><h2>Entre na sua fazenda</h2><p>Entre com sua senha ou receba um link de acesso por e-mail.</p></div><div className="auth-choices"><form action={signInWithPassword}><label htmlFor="password-email">Seu e-mail</label><input id="password-email" name="email" type="email" autoComplete="username" placeholder="voce@exemplo.com" required/><label htmlFor="password">Sua senha</label><input id="password" name="password" type="password" autoComplete="current-password" required/><button type="submit">Entrar com senha <span>↗</span></button></form><div className="auth-divider">Ou receba um link para entrar / criar sua senha</div><form action={signIn}><label htmlFor="email">Seu e-mail</label><input id="email" name="email" type="email" autoComplete="email" placeholder="voce@exemplo.com" required/><button type="submit">Enviar link de acesso <span>↗</span></button></form></div></section>
      : farmsError ? null
